...
use stm32wlxx_hal::{
    self as hal,
    adc::{self, Adc, Clk},
    cortex_m::{self, delay::Delay, interrupt::CriticalSection},
    gpio::{pins, PortA},
...
fn main() -> ! {
    ...
    cortex_m::interrupt::free(|cs: &CriticalSection| unsafe {
    ...
    let vbat: f32 = (3.3 * f32::from(sample) * 3.0) / 4096.0;
    ...
    let mut lpuart: LpUart<pins::A3, pins::A2> =
